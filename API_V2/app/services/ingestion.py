...
class DANEPopulationConnector:
    ...
    def download(self, url: str, source_id: str) -> DownloadResult:
        ...
        with httpx.stream(
            ...
            headers={"User-Agent": "Databolico-GenZ-API-V2/2.0"},
        ) as response:
    ...
    def parse(
        ...
        if len(age_columns) != expected:
            raise ValueError(f"Expected {expected} sex-age columns, found {len(age_columns)}")
        ...
        for row in sheet.iter_rows(min_row=self.HEADER_ROW + 1, values_only=True):
            ...
            if municipality_code not in municipality_codes or area != "Total":
                continue
